...
def get_ne_name_rnc_ipa(tn, ip):	
	file_tmp = tmp_local_file(ip)	
	ret = "-"
	fp = open(file_tmp, "wb")
	fp.write(send_dx_command(tn, ["ZDDE::\"ZL:1\",\"ZLE:1,RUOSTEQX\",\"Z1C\";"], ["COMMAND EXECUTED"]))
	fp.close()
	...
	def __init__(self, ip, tn, name, logdir, targetid):
		...
		try:
			...
			cmdlist = ["set cli built-in rows -1",
			    "show radio-network omsconn", 
			    "show license target-id", 
				"show license all",
			...
			loglist = [workdir + os.sep + "set_cli_built.log",
				workdir + os.sep + "omscon.log",
				workdir + os.sep + "licence_target_id.log",
			...
			if tna:
				...
				if os.path.isdir(work_dire):
					...
					if "BSC" in ne_type:				
						...
						rnc_thread = threading.Thread(target=mgw, args=(ip, tna, ne_name, logname))						
						rnc_thread.start()
