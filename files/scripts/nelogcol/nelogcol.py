...
key = raw_input("Enter the key to decrypt the passwords:")
...
for l in fp.readlines():	
	...
	if not l.startswith("#"):				
		...
		if par_cont is 3:			
			...
			enc_pw = l.strip().split(",")[2]			
			usr = cipher.decrypt(base64.b64decode(enc_pw)).strip().split(",")[0]
			pwd = cipher.decrypt(base64.b64decode(enc_pw)).strip().split(",")[1]
			_ne.set_user(usr)
			_ne.set_password(pwd)
			nelist.append(_ne)
...
os.chdir(LOG_DIRE)
for f in listdir("."):	
	if not(isfile(join(LOG_DIRE, f))) and not "_failed" in f:
		try:
			zipdir(f, zipf)
		except:
